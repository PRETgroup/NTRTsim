...
/**
 * @file tgZeroMQController.cpp
 * @brief Implementation of the tgZeroMQController class. 
 * @author Hammond Pearce
 * @date June 2019
 * $Id$
 */

#include "tgZeroMQController.h"
...
tgZeroMQController::tgZeroMQController(tgBasicActuator* controllable, double setPoint) :
m_sca(controllable),
...
tgZeroMQController::~tgZeroMQController()
{
    m_sca = NULL;
}
	
void tgZeroMQController::control(double dt)
{
	if (dt <= 0.0)
	{
...
void tgZeroMQController::control(double dt, double setPoint, double sensorData)
{
	if (dt <= 0.0)
	{
		throw std::runtime_error ("Timestep must be positive.");
...
void tgZeroMQController::control(tgBasicActuator& sca, double dt, double setPoint)
{
    if (dt <= 0.0)
	{
		throw std::runtime_error ("Timestep must be positive.");
