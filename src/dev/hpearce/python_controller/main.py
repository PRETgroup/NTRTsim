...
def main():
    ...
    lif_model2 = nengo_one_osc_no_readout.get_model(robot, w_set, True, gauss_std = 0.09)
    
    print('Robot created')

    experiments = [
        ("Nengo LIF (0.05)", lif_model),
        ("Nengo LIF (0.09)", lif_model2),
    ...
    if noisy_sine:
        ...
        while robot.lifetime < simulation_time:
            ...
            if noise < -1:
                noise = -1
            osc = math.sin(2*math.pi*sTime) * (1 + noise)
        ...
        result = ('Noisy Sinusoid (0.05)', [pos[::2] for pos in robot.com_history])
        if save_csv:
            with open(dt_string + '_results'+str(exp_count)+'_w=' + str(result[0])+'.csv', 'w') as file:
                file.write('row,col\n')
                positions = result[1]
        ...
        master_osc = [0] * 8   
        gauss_dist = nengo.dists.Gaussian(mean=0,std=0.09)     
        ...
        print("\nNoisy sine simulation done.")
        result = ('Noisy Sinusoid (0.09)', [pos[::2] for pos in robot.com_history])
        if save_csv:
            with open(dt_string + '_results'+str(exp_count)+'_w=' + str(result[0])+'.csv', 'w') as file:
                file.write('row,col\n')
                positions = result[1]
