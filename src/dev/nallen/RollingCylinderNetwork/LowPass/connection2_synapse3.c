...
void Connection2Synapse3Run(Connection2Synapse3* me) {
    ...
    enum Connection2Synapse3States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE3_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE3_L: // Intra-location logic for state l
