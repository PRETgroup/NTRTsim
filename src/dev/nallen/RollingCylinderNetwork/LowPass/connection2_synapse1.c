...
void Connection2Synapse1Run(Connection2Synapse1* me) {
    ...
    enum Connection2Synapse1States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE1_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE1_L: // Intra-location logic for state l
