...
void Connection2Synapse9Run(Connection2Synapse9* me) {
    ...
    enum Connection2Synapse9States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE9_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE9_L: // Intra-location logic for state l
