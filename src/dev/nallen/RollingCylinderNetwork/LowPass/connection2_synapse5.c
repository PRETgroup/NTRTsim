...
void Connection2Synapse5Run(Connection2Synapse5* me) {
    ...
    enum Connection2Synapse5States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE5_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE5_L: // Intra-location logic for state l
