...
void Connection2Synapse4Run(Connection2Synapse4* me) {
    ...
    enum Connection2Synapse4States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE4_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE4_L: // Intra-location logic for state l
