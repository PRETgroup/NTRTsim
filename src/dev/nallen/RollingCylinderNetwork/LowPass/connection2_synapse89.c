...
void Connection2Synapse89Run(Connection2Synapse89* me) {
    ...
    enum Connection2Synapse89States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE89_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE89_L: // Intra-location logic for state l
