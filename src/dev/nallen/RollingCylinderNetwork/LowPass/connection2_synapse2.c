...
void Connection2Synapse2Run(Connection2Synapse2* me) {
    ...
    enum Connection2Synapse2States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE2_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE2_L: // Intra-location logic for state l
