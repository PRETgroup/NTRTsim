...
void Connection2Synapse65Run(Connection2Synapse65* me) {
    ...
    enum Connection2Synapse65States state_u = me->state;
    double out_u = me->out;
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE65_L: // Logic for state l
            break;
    }
    ...
    switch(me->state) {
        case CONNECTION2_SYNAPSE65_L: // Intra-location logic for state l
