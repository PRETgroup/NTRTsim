...
void Ensemble093Run(Ensemble093* me) {
    ...
    enum Ensemble093States state_u = me->state;
    ...
    switch(me->state) {
        case ENSEMBLE0_93_INTEGRATE: // Logic for state integrate
            if(me->v == 1.0) {
                v_u = 0.0;
                spike_u = true;
                t_u = 0.0;
                ...
                // Next state is spike
                state_u = ENSEMBLE0_93_SPIKE;
            }
            else if(me->v == 0.0 && me->in < 0.0) {
                v_u = 0.0;

                // Next state is limit
                state_u = ENSEMBLE0_93_LIMIT;
            }
            break;
        case ENSEMBLE0_93_LIMIT: // Logic for state limit
            if(me->in >= 0.0) {
                v_u = 0.0;

                // Next state is integrate
                state_u = ENSEMBLE0_93_INTEGRATE;
            }
            break;
        case ENSEMBLE0_93_SPIKE: // Logic for state spike
            if(me->t == 0.002) {
                spike_u = false;
                ...
                state_u = ENSEMBLE0_93_REFRACTORY;
            }
            break;
        case ENSEMBLE0_93_REFRACTORY: // Logic for state refractory
            if(me->t == 0.002) {
                // Next state is integrate
                state_u = ENSEMBLE0_93_INTEGRATE;
            }
    ...
    switch(me->state) {
        case ENSEMBLE0_93_INTEGRATE: // Intra-location logic for state integrate
            ...
            break;
        case ENSEMBLE0_93_LIMIT: // Intra-location logic for state limit
            v_u = 0.0;
        ...
        case ENSEMBLE0_93_SPIKE: // Intra-location logic for state spike
            ...
            break;
        case ENSEMBLE0_93_REFRACTORY: // Intra-location logic for state refractory
            t_u = me->t + 1.0 * STEP_SIZE;
